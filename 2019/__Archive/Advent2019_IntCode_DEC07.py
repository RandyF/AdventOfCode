class ADV19_IntcodeComputer(object):
    ...
    def __init__(self, sIntcodeProg = [], lArgs = [], iVerbosity = 0 ): 
    ...
    def DecodeIncodeMEM(self, printIntCode = [], iWidth = 20):
        if printIntCode == []:
            pIC = self.IntcodeMEM
        else:
            pIC = printIntCode
    ...
    def GetIntParams(self, lCmd = [], nParams = 0):
        ...
        #print( "Extended Opcode: ", sOpCode
        
        sModes = sOpCode[:nParams]
        #print( "Modes: ", sModes

        lParams = []
        tParam = 0
        #print( range(len(sModes))
        for i in range(len(sModes)):
            iPos = len(sModes) - i - 1
            #print( i, iPos, sModes[iPos]
            if sModes[iPos] == '0':
    ...
    def IntOp_01(self, lCmd = [] ):
            if self.Verbosity >= 5: print( "   ADDER")
            ...
            if self.Verbosity >= 10: print( " Adding %d to %d and sending to *%d" % (tParams[0], tParams[1], tParams[2])) 
    ...
    def IntOp_02(self, lCmd = [] ):
            if self.Verbosity >= 5: print( "   MULTIPLIER")
            ...
            tParams = self.GetIntParams( lCmd, 2 )
            if self.Verbosity >= 10: print( " Multiplying %d by %d and sending to *%d" % (tParams[0], tParams[1], tParams[2]) )
    ...
    def IntOp_03(self, lCmd = [] ):
        ...
        # it at address 50.
        if self.Verbosity >= 5: print( "   INPUT to", lCmd[1])
    ...
    def IntOp_04(self, lCmd = [] ):
        ...
        if self.Verbosity >= 5: print( "   OUTPUT")
        tParams = self.GetIntParams( lCmd, 1 )
        #print( tParams
        self.StdOut.append(tParams[0])
        if self.Verbosity >= 1: print( "Output: [", tParams[0] , "]" )
        self.IP += len(lCmd)
        #self.print(IncodeMEM()
    ...
    def IntOp_05(self, lCmd = [] ):
        ...
        if self.Verbosity >= 5: print( "   JUMP IF TRUE", lCmd)
    ...
    def IntOp_06(self, lCmd = [] ):
        ...
        # pointer to the value from the second parameter. Otherwise, it does nothing.
        if self.Verbosity >= 5: print( "   JUMP IF FALSE", lCmd)
    ...
    def IntOp_07(self, lCmd = [] ):
        ...
        if self.Verbosity >= 5: print( "   LESS THAN", lCmd)
    ...
    def IntOp_08(self, lCmd = [] ):
        ...
        if self.Verbosity >= 5: print( "   EQUAL", lCmd)
    ...
    def ProcessCurrentInstruction(self):
        ...
        if self.MachineState == 1: self.ProcessCurrentInstruction()
